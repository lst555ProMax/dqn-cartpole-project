...
import time # 引入 time 模块 for timestamp
from alive_progress import alive_bar # 引入 alive_bar 用于整体进度

# 更新后的超参数 (来自Optuna优化结果)
BATCH_SIZE = 32
LR = 0.007450981254753364
GAMMA = 0.9160294715062804
TARGET_NETWORK_REPLACE_FREQ = 50
MEMORY_CAPACITY = 3000
EPSILON_START = 0.8694743685127261
EPSILON_END = 0.06223423318523694
EPSILON_DECAY_STEPS = 1500
HIDDEN_SIZE = 128
maximum_episode_length = 500

# 定义最大训练回合数常量，用于失败时计入总和
MAX_EPISODES_PER_RUN = 200  # 从 run_single_experiment_ddqn 函数中提取的常量
...
class DDQN(object):  # 类名可以改为 DDQN 以示区别，或者保持 DQN 也可以
    ...
    def learn(self):
        ...
        # 1. Select action using eval_net for s_
        # We detach eval_net's output here too, as its action selection is part of the target calculation, not directly optimized
        q_eval_next_actions_indices = self.eval_net(b_s_).detach().max(1)[1].view(BATCH_SIZE, 1)
        # 2. Evaluate the Q-value of that action using target_net for s_
...
# 该函数返回 0-indexed achieved_episode 或 None
def run_single_experiment_ddqn(env_instance, n_states, n_actions, env_a_shape):
    # 使用 DDQN 类
    ddqn_agent = DDQN(n_states, n_actions, env_a_shape)
    ...
    for i_episode in range(MAX_EPISODES_PER_RUN):
        ...
        while True:
            ...
            # Note: This failure penalty might interfere with standard rewards/learning
            # if done and step_count < maximum_episode_length - 1:  # 如果提前失败，给一个大的负奖励
            #     modified_r = -100
            # Keeping original logic as per instruction
            ...
            if ddqn_agent.memory_counter > BATCH_SIZE: # Changed condition from > MEMORY_CAPACITY to > BATCH_SIZE as is typical
                 ddqn_agent.learn()

            # Check for reaching frame limit first for the success condition
            # Note: CartPole-v1 truncated is True at step 500 (0-indexed step_count 499)
            if step_count + 1 >= maximum_episode_length:
                 # print(f'Reached {maximum_episode_length} frames at episode {i_episode}!') # 保持外部打印
                 achieved_episode = i_episode # Record 0-indexed episode number
                 break # Stop episode loop

            # Check for natural termination (pole fell, went out of bounds, or truncated)
...
if __name__ == "__main__":
    ...
    # Assuming script is in dqn_algorithm/dqn_test3.py and output is in dqn_output/dqn_test3.txt
    ...
    results_filename = os.path.join(output_dir, "dqn_test4.txt") # Use os.path.join

    # Open the output file in WRITE mode ('w') to overwrite previous content
    with open(results_filename, "w") as f: # Changed "a" to "w"
        f.write("===== DDQN 训练结果报告 (使用优化参数) =====\n")  # 标明是DDQN
        ...
        f.write(f"评价指标: 计算所有运行的平均达成episode（失败记为{MAX_EPISODES_PER_RUN}）\n")
        f.write("-" * 50 + "\n")
        ...
        # Add outer alive_bar for the 30 runs
        with alive_bar(N_RUNS, title="Overall Progress") as bar:
             for run_id in range(N_RUNS):
                print(f"\n--- Starting DDQN Run {run_id + 1}/{N_RUNS} ---")  # 标明是DDQN Run
                f.write(f"  第{run_id + 1:02d}次  |")

                # Run a single experiment
                achieved_episode_in_run = run_single_experiment_ddqn(env_main, N_STATES_MAIN, N_ACTIONS_MAIN,
                                                                     ENV_A_SHAPE_MAIN) # This returns 0-indexed episode or None

                # Process result and populate all_run_episode_counts
                if achieved_episode_in_run is not None:
                    ...
                    print(f"DDQN Run {run_id + 1}: Success, solved in {achieved_episode_in_run + 1} episodes.") # Print 1-based
                else:
                    # If failed, append the maximum number of episodes allowed for the run
                    all_run_episode_counts.append(MAX_EPISODES_PER_RUN)
                    f.write(f"    失败    |     --    \n")
                    print(f"DDQN Run {run_id + 1}: Failed to solve within {MAX_EPISODES_PER_RUN} episodes.")

                bar() # Update outer bar
        ...
        if all_run_episode_counts: # Check if any runs were attempted
            ...
            f.write(f"\n平均达成episode (总体, 失败记为{MAX_EPISODES_PER_RUN}): {avg_episode_overall:.1f}\n") # Updated description
        ...
        if success_count > 0:
            ...
            print(f"\nSummary: No run successfully solved the environment within {MAX_EPISODES_PER_RUN} episodes.")

        f.write("\n===== DDQN 报告结束 =====")

    # --- MODIFICATION END ---

    print(f"\nDDQN run report saved to {results_filename}")