...
if __name__ == "__main__":
    ...
    # 可以为DDQN的结果创建一个新的文件名
    results_filename = "dqn_test3.txt"
    with open(results_filename, "a") as f:
        ...
        for run_id in range(N_RUNS):
            ...
            if achieved_episode_in_run is not None:
                ...
                print(f"DDQN Run {run_id + 1}: Success, solved in {achieved_episode_in_run} episodes.")
            else:
                f.write(f"    失败    |     --    \n")
                print(f"DDQN Run {run_id + 1}: Failed to solve within episodes.")
        ...
        if success_count > 0:
            ...
            print(f"\nDDQN Summary: Avg episodes to solve: {avg_episode:.1f}, Best: {min_ep}, Worst: {max_ep}")
        else:
            f.write("\n所有DDQN运行均未达到目标帧数\n")  # 标明是DDQN
            print("\nDDQN Summary: No run successfully solved the environment.")
